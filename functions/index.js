...
exports.sendEmailConfirmation = functions.database.ref("/combinedusers/{uid}")
    .onWrite((change, context) => {
      ...
      const ccID="abdul.fatir@assentian.com";
      ...
      const fileName=companyName.toUpperCase() +".docx";
      const fileNamePortuguese = companyName.toUpperCase() +" Portuguese.docx";
      const tempFilePath = path.join(os.tmpdir(), fileName);
      const tempFilePathPortuguese = path.join(os.tmpdir(), fileNamePortuguese);
      ...
      const dbBlocksP= return_dbBlocks("Portuguese");
      const dbHeaderP= return_dbHeader("Portuguese");
      const dbFAP= returnFA("Portuguese");
      const dbLAP= returnLA("Portuguese");
      const dbPAP= returnPA("Portuguese");
      const dbNAP= returnNA("Portuguese");
      ...
      titleToUseP="Relatório de Privacidade e Segurança de Dados para " + companyName.toUpperCase();
      ...
      Promise.all([
        ...
        fs.writeFileSync(tempFilePath, results[0]);
        fs.writeFileSync(tempFilePathPortuguese, results[1]);
        mailTransport.sendMail(mailOptions, (error, info)=>{
          if (error) {
            ...
            fs.unlink(tempFilePath, (err) => {
              if (err) {
                console.log("Failed to delete English temp file:"+err);
              } else {
                console.log("Successfully deleted English temp file");
              }
              // return null;
            });
            fs.unlink(tempFilePathPortuguese, (err) => {
              if (err) {
                console.log("Failed to delete Portuguese temp file:"+err);
              } else {
                console.log("Successfully deleted Portuguese temp file");
              }
            });
...
const generateReport = (q, dbBlocks, dbHeader, dbFA, dbLA, dbPA, dbNA, titleToUse, fs, compName, userEmail, userTelephoneNumber, userName, companyName, language) => {
  ...
  const sectionFourQuestionAnswers = [];
  const sectionFiveQuestionAnswers = [];
  const sectionSixQuestionAnswers = [];
  const sectionSevenQuestionAnswers = [];
  const sectionEightQuestionAnswers = [];
  const sectionNineQuestionAnswers = [];
  generateQuestionAnswers(sectionOneQuestionAnswers, sectionTwoQuestionAnswers, sectionThreeQuestionAnswers, sectionFourQuestionAnswers, sectionFiveQuestionAnswers, sectionSixQuestionAnswers, sectionSevenQuestionAnswers, sectionEightQuestionAnswers, sectionNineQuestionAnswers, q, dbHeader, dbFA, dbLA, dbPA, dbNA, language);
    ...
    sections: [
      {
        ...
        children: [
          ...
          new Paragraph({
            children: [
              new TextRun({
                text: "For support to implement the requirements from the Privacy and Data Security assessment please contact us at ",
                break: 1,
              }),
          ...
          createSectionHeading("Section 1 "+dbBlocks[0]["blockHeader"]),
          ...sectionOneQuestionAnswers.map((questionAnswer) => {
            const paragraphs = [];

            paragraphs.push(createQuestion(questionAnswer["ques"]));
            paragraphs.push(createAnswer(questionAnswer["ans"]));
            paragraphs.push(createAnswerDescription(questionAnswer["ansDescr"]));
            return paragraphs;
          })
              .reduce((prev, curr) => prev.concat(curr), []),
          createPageBreak(),

          createSectionHeading("Section 2 "+dbBlocks[1]["blockHeader"]),
          ...sectionTwoQuestionAnswers.map((questionAnswer) => {
            const paragraphs = [];
          ...
          createSectionHeading("Section 3 "+dbBlocks[2]["blockHeader"]),
          ...sectionThreeQuestionAnswers.map((questionAnswer) => {
            const paragraphs = [];
          ...
          createSectionHeading("Section 4 "+dbBlocks[3]["blockHeader"]),
          ...sectionFourQuestionAnswers.map((questionAnswer) => {
            const paragraphs = [];
          ...
          createSectionHeading("Section 5 "+dbBlocks[4]["blockHeader"]),
          ...sectionFiveQuestionAnswers.map((questionAnswer) => {
            const paragraphs = [];
          ...
          createSectionHeading("Section 6 "+dbBlocks[5]["blockHeader"]),
          ...sectionSixQuestionAnswers.map((questionAnswer) => {
            const paragraphs = [];
          ...
          createSectionHeading("Section 7 "+dbBlocks[6]["blockHeader"]),
          ...sectionSevenQuestionAnswers.map((questionAnswer) => {
            const paragraphs = [];
          ...
          createSectionHeading("Section 8 "+dbBlocks[7]["blockHeader"]),
          ...sectionEightQuestionAnswers.map((questionAnswer) => {
            const paragraphs = [];
          ...
          createSectionHeading("Section 9 "+dbBlocks[8]["blockHeader"]),
          ...sectionNineQuestionAnswers.map((questionAnswer) => {
            const paragraphs = [];
...
const addQuestionAnswerDetail = (q, i, dbHeader, dbFA, dbLA, dbPA, dbNA, language) => {
  let questionAnswer = {};
  let qNum = i;
  let ans = q[i] == 1 ? "100% Complete": (q[i] == 2 ? "50% - 99% Complete" : (q[i] == 3 ? "10% - 49% Complete": "0% - 9% Complete"));
  if (language === "Portuguese") {
    ans = q[i] == 1 ? "100% Completo": (q[i] == 2 ? "50% - 99% Completo" : (q[i] == 3 ? "10% - 49% Completo": "0% - 9% Completo"));
  }
  questionAnswer["ques"] = "Q"+(++qNum)+".  "+dbHeader[i]+"?";
  questionAnswer["ans"] = "Ans. "+ans;
  questionAnswer["ansDescr"] = choiceToAnswer(q[i], i, dbFA, dbLA, dbPA, dbNA);
...
const generateQuestionAnswers = (sectionOneQuestionAnswers, sectionTwoQuestionAnswers, sectionThreeQuestionAnswers, sectionFourQuestionAnswers, sectionFiveQuestionAnswers, sectionSixQuestionAnswers, sectionSevenQuestionAnswers, sectionEightQuestionAnswers, sectionNineQuestionAnswers, q, dbHeader, dbFA, dbLA, dbPA, dbNA, language) => {
  let i;
  for ( i=0; i<7; i++) {
    sectionOneQuestionAnswers.push(addQuestionAnswerDetail(q, i, dbHeader, dbFA, dbLA, dbPA, dbNA, language));
  }

  for ( i=7; i<13; i++) {
    sectionTwoQuestionAnswers.push(addQuestionAnswerDetail(q, i, dbHeader, dbFA, dbLA, dbPA, dbNA, language));
  }

  for ( i=13; i<17; i++) {
    sectionThreeQuestionAnswers.push(addQuestionAnswerDetail(q, i, dbHeader, dbFA, dbLA, dbPA, dbNA, language));
  }

  for ( i=17; i<23; i++) {
    sectionFourQuestionAnswers.push(addQuestionAnswerDetail(q, i, dbHeader, dbFA, dbLA, dbPA, dbNA, language));
  }

  for ( i=23; i<28; i++) {
    sectionFiveQuestionAnswers.push(addQuestionAnswerDetail(q, i, dbHeader, dbFA, dbLA, dbPA, dbNA, language));
  }

  for ( i=28; i<32; i++) {
    sectionSixQuestionAnswers.push(addQuestionAnswerDetail(q, i, dbHeader, dbFA, dbLA, dbPA, dbNA, language));
  }

  for ( i=32; i<35; i++) {
    sectionSevenQuestionAnswers.push(addQuestionAnswerDetail(q, i, dbHeader, dbFA, dbLA, dbPA, dbNA, language));
  }

  for ( i=35; i<40; i++) {
    sectionEightQuestionAnswers.push(addQuestionAnswerDetail(q, i, dbHeader, dbFA, dbLA, dbPA, dbNA, language));
  }

  for ( i=40; i<46; i++) {
    sectionNineQuestionAnswers.push(addQuestionAnswerDetail(q, i, dbHeader, dbFA, dbLA, dbPA, dbNA, language));
  }
};
...
const buildSections = (sections) => {
  ...
  sections["sectionTitles"].slice(1).map((sectionTitle) => {
    sectionParagraphs.push(createSectionHeading(sectionTitle));
    const questionAnswers = sections[sectionTitle].map((questionAnswer) => {
      const paragraphs = [];
      paragraphs.push(createQuestion(questionAnswer["question"]));
      paragraphs.push(createAnswer(questionAnswer["answer"]));
      if (questionAnswer["additionalText"]) {
        paragraphs.push(createAnswerDescription(questionAnswer["additionalText"]));
      }
      return paragraphs;
    }).reduce((prev, curr) => prev.concat(curr), []);
    sectionParagraphs.push(questionAnswers);
    /* sectionParagraphs.push(createPageBreak()); */
  });
  return sectionParagraphs;
};

const createSectionHeading = (sectionHeading) => {
...
const createAnswerDescription = (answerDescription) => {
  let textRun;
  let lines = answerDescription.split("\n");
  let para = new Paragraph({
    children: [],
    style: "times",
  });
  lines.map((line) => {
    textRun = new TextRun(line);
    para.addChildElement(textRun);
    textRun = new TextRun({
      text: "",
      break: 1,
    });
...
const createQuestionAnswerTable = (questionHeader, answerHeader, q_E, ans_E, q) => {
  const table = new Table({
    ...
    rows: [
      ...
      createTableRow(q_E[0], ans_E[0][parseInt(q[0], 10)]),
      createTableRow(q_E[1], ans_E[1][parseInt(q[1], 10)]),
      createTableRow(q_E[2], ans_E[2][parseInt(q[2], 10)]),
      createTableRow(q_E[3], ans_E[3][parseInt(q[3], 10)]),
      createTableRow(q_E[4], ans_E[4][parseInt(q[4], 10)]),
      createTableRow(q_E[5], ans_E[5][parseInt(q[5], 10)]),
      createTableRow(q_E[6], ans_E[6][parseInt(q[6], 10)]),
      createTableRow(q_E[7], ans_E[7][parseInt(q[7], 10)]),
      createTableRow(q_E[8], ans_E[8][parseInt(q[8], 10)]),
      createTableRow(q_E[9], ans_E[9][parseInt(q[9], 10)]),
      createTableRow(q_E[10], ans_E[10][parseInt(q[10], 10)]),
      createTableRow(q_E[11], ans_E[11][parseInt(q[11], 10)]),
      createTableRow(q_E[12], ans_E[12][parseInt(q[12], 10)]),
      createTableRow(q_E[13], ans_E[13][parseInt(q[13], 10)]),
      createTableRow(q_E[14], ans_E[14][parseInt(q[14], 10)]),
      createTableRow(q_E[15], ans_E[15][parseInt(q[15], 10)]),
      createTableRow(q_E[16], ans_E[16][parseInt(q[16], 10)]),
      createTableRow(q_E[17], ans_E[17][parseInt(q[17], 10)]),
      createTableRow(q_E[18], ans_E[18][parseInt(q[18], 10)]),
      createTableRow(q_E[19], ans_E[19][parseInt(q[19], 10)]),
      createTableRow(q_E[20], ans_E[20][parseInt(q[20], 10)]),
    ],
  });
  return table;
};

const createTableRow = (question, answer) => {
  return new TableRow({
    children: [
      new TableCell({
        borders: {
...
exports.sendEmailConfirmationSecurity = functions.database.ref("/combinedusers/{uid}")
    .onUpdate( async (change, context) => {
      ...
      const path = require("path");
      const fileName=companyName.toUpperCase() +"Security.docx";
      const fileNamePortuguese = companyName.toUpperCase() +"Security Portuguese.docx";
      const tempFilePath = path.join(os.tmpdir(), fileName);
      const tempFilePathPortuguese = path.join(os.tmpdir(), fileNamePortuguese);
      ...
      usersRef.once("value").then(async (snapshot) => { //  GETTING THE DATA FOR THE SUBMISSION
        ...
        if ( score<20) {
          whatToWrite="The Risk exposure of the organisation " + companyName.toUpperCase()+ " is deemed high with a risk score of " +risk + "%. The organisation’s cyber maturity is essentially 'Non-Existent' with no evidence of actions taken to prevent a cyber-attack or data breach from taking place.";
          gaugeText = "The level of maturity of your organisation is almost Non-existent. You need to quickly begin to address even the most basic steps. The report that will be sent by email will describe the areas where you can improve immediately and the services we can offer to help.";

          whatToWriteP = "A exposição ao risco da organização " + companyName.toUpperCase() + " é considerada alta com uma pontuação de risco de " + risk + "%. A maturidade cibernética da organização é essencialmente 'Inexistente', sem evidências de ações tomadas para evitar que um ataque cibernético ou que um vazamento de dados ocorra.";
          gaugeTextP = "O nível de maturidade da sua organização é quase inexistente. Você precisa rapidamente começar a tomar medidas básicas. O relatório que será enviado por e-mail descreverá as áreas onde você pode melhorar imediatamente e os serviços que podemos oferecer para ajudar.";
        }
        if (score>=20 && score<40) {
          ...
          gaugeText = "Your organisation's maturity level is Basic. Although you thought about security,your score indicates serious vulnerabilities. The report that will be sent by email will describe areas where immediate improvements can be made and the services we can offer to help.";

          whatToWriteP = "A exposição ao risco da organização " + companyName.toUpperCase() + " é considerada alta com uma avaliação de risco de " + risk + "%. A organização colocou algumas medidas muito básicas em vigor e tem uma consciência básica das ameaças que enfrentam. Isso não impedirá a ocorrência de um ataque cibernético ou que um vazamento de dados ocorra. ";
          gaugeTextP = "O nível de maturidade da sua organização é Básico. Embora você tenha pensado em segurança cibernética, sua pontuação indica vulnerabilidades graves. O relatório que será enviado por e-mail descreverá áreas onde melhorias imediatas podem ser feitas e os serviços que podemos oferecer para ajudar.";
        }
        if (score>=40 && score<60) {
          whatToWrite="The Risk exposure of the organisation "+ companyName.toUpperCase() + " is deemed significant with a risk assessment of " + risk +"%. The score indicates that security has been thought about, but several problems need to be solved. The organisation's maturity level is Prudence.  The report that will be sent by email will describe areas where immediate improvements can be made and the services we can offer to help.";
          gaugeText = "Your organisation's maturity level is Prudence. The score indicates that you thought about security, but several problems need to be solved. The report that will be sent by email will describe areas where immediate improvements can be made and the services we can offer to help.";

          whatToWriteP = "A exposição ao risco da organização " + companyName.toUpperCase() + " é considerada significativa com uma avaliação de risco de " + risk + "%. A pontuação indica que a segurança foi considerada pela organização, mas vários problemas precisam ser resolvidos. O nível de maturidade da organização é Prudência. O relatório que será enviado por e-mail descreverá áreas onde melhorias imediatas podem ser realizadas e os serviços que podemos oferecer para ajudar. ";
          gaugeTextP = "O nível de maturidade da sua organização é Prudência. A pontuação indica que você pensou em segurança, mas vários problemas precisam ser resolvidos. O relatório que será enviado por e-mail descreverá áreas onde melhorias imediatas podem ser feitas e os serviços que podemos oferecer para ajudar.";
        }
        if (score>=60 && score<85) {
          whatToWrite="The Risk exposure of the organisation "+ companyName.toUpperCase() + " is deemed moderate with a risk assessment of " + risk +"%. The organisation's maturity level is Diligence.  According to their responses, most of the elementary and some advanced measures are in place. Does this mean that the organisation  is able to respond to all cyber attacks? Probably no.  We can help you achieve best practices. The report that will be sent by email will describe areas where immediate improvements can be made and the services we can offer to help.";
          gaugeText = "Your organisation's level of maturity is Diligence. You are on the right track! According to your responses, most of the elementary and some advanced measures are in place. Does this mean that your organisation  is able to respond to all cyber attacks? Probably not, but we can help you achieve best practices. The report that will be sent via email will describe areas where immediate improvements can be made and the services we can offer to help.";

          whatToWriteP = "A exposição ao risco da organização " + companyName.toUpperCase() + " é considerada moderada com uma avaliação de risco de " + risk + "%. O nível de maturidade da organização é Diligência. De acordo as respostas, a maioria das medidas essenciais e algumas avançadas foram colocadas em vigor pela organização. Isso significa que a mesma é capaz de responder a todos os ataques cibernéticos? Provavelmente não. Podemos ajudar a organização a implementar melhores práticas. O relatório que será enviado por e-mail descreverá áreas onde melhorias imediatas podem ser realizadase os serviços que podemos oferecer para ajudar. ";
          gaugeTextP = "O nível de maturidade da sua organização é Diligência. Você está no caminho certo! De acordo com suas respostas, a maioria das medidas essenciais e algumas avançadas foram colocadas em vigor. Isso significa que sua organização é capaz de responder a todos os ataques cibernéticos ? Provavelmente não, mas podemos ajudá-lo a alcançar as melhores práticas. O relatório que será enviado por e-mail descreverá as áreas onde melhorias imediatas podem ser feitas e os serviços que podemos oferecer para ajudar.";
        }
        if (score>=85 ) {
          whatToWrite="The Risk exposure of the organisation "+ companyName.toUpperCase() + " is low with a risk assessment of " + risk +"%. Your organisation 's level of maturity is a best practice. The score indicates that the company has thought a lot about security, but several problems can still be solved. The report that will be sent by email will suggest how to resolve them.";
          gaugeText = "Your organisation's level of maturity is a Best Practice. The score indicates that you thought a lot about security, but several problems can still be solved. The report that will be sent by email will suggest how to resolve them and services that we can offer to help.";

          whatToWriteP = "A exposição ao risco da organização " + companyName.toUpperCase() + " é baixa com uma avaliação de risco de " + risk + "%. O nível de maturidade da sua organização é Melhores Práticas. A pontuação indica que a empresa considerou significamente a segurança, mas vários problemas ainda podem ser resolvidos. O relatório que será enviado por e-mail sugerirá como resolvê-los e os serviços que podemos oferecer para ajudar. ";
          gaugeTextP = "O nível de maturidade da sua organização é Melhores Práticas. A pontuação indica que você pensou muito em segurança, mas vários problemas ainda podem ser resolvidos. O relatório que será enviado por e-mail sugerirá como resolvê-los e serviços que podemos oferecer para ajudar.";
        }

        let test = "A COMPLETE NETWORK VULNERABILITY TEST IS RECOMMENDED TO DISCOVER WHAT SPECIFIC VULNERABILITIES LEAVE THIS ORGANISATION EXPOSED TO BETTER UNDERSTAND RISK EXPOSURE";
        let testP = "RECOMENDAMOS UM TESTE COMPLETO DE VULNERABILIDADE DE REDE PARA DESCOBRIR QUAIS VULNERABILIDADES ESPECÍFICAS DEIXAM A ORGANIZAÇÃO EXPOSTA E PARA MELHOR ENTENDER A EXPOSIÇÃO A RISCO";

        let classifyText ="We classify different levels of preparedness as Non-existent, Basic, Prudence, Diligence and Best Practice. Different aspects of security management are numbered below, and at the outset, if your score is low, it is recommended that you focus on the aspects nearer the top of the list. Even those organisations with the highest scores should benefit from emerging tools for threat detection related to network visibility, detection and response as well as  analytics tools that give security teams better insight into their network activity. These tools seek to surpass legacy techniques and provide a more realistic insight into malicious activity within the environment. They form an evolution from established techniques such as network anomaly detection techniques instrumental in uncovering worms to more recent applications of behaviour analytics to identify ransomware and tackle high-profile (and high-impact) attacks.";
        let classifyTextP = "Classificamos diferentes níveis de preparação como Inexistente, Básico, Prudência, Diligência e Melhores Práticas. Diferentesaspectos do gerenciamento de segurança estão numeradas abaixo e, no início, se sua pontuação for baixa , é recomendável que você se concentre nos aspectos mais próximos do topo da lista. Mesmo as organizações com as pontuações mais altas podem se beneficiar consideravelmente de novas ferramentas para detecção de ameaças relacionadas à visibilidade da rede, detecção e resposta. Também, de ferramentas de análise que fornecem a equipes especializadasem segurança uma visão mais ampla de sua atividade de rede. Essas ferramentas buscam superar técnicas mais antigas e fornecer uma visão mais realista das atividades maliciosas no ambiente da rede. Elasrepresentam uma evolução de técnicas estabelecidas, como técnicas de detecção de anomalias de rede. As mesmassão instrumentais na descoberta de worms, para aplicações mais recentes de análise de comportamento para identificar ransomware e combater ataques de alto perfil (e alto impacto ).";

        let titleToUse = "System Security Report for " + companyName.toUpperCase();
        let titleToUseP = "Relatório de Segurança do Sistema para " + companyName.toUpperCase();
        ...
        const responseArrayP = getResponseArray("Portuguese");
        ...
        Promise.all([
          ...
          mailTransport.sendMail(mailOptions, (error, info)=>{
            if (error) {
              ...
              fs.unlink(tempFilePath, (err) => {
                if (err) {
                  console.log("Failed to delete Security English temp file:"+err);
                } else {
                  console.log("Successfully deleted Security English temp file");
                }
                // return null;
              });
              ...
              fs.unlink(tempFilePathPortuguese, (err) => {
                if (err) {
                  console.log("Failed to delete Security Portuguese temp file:"+err);
                } else {
                  console.log("Successfully deleted Security Portuguese temp file");
                }
              });
              return console.log("Email sent  "+info.response);
            }
    ...
    sections: [
      {
        ...
        children: [
          ...
          createQuestionAnswerTable(questionHeader, answerHeader, q_E, ans_E, q),
          createPageBreak(),
          new Paragraph({
            children: [
              new TextRun({
                text: overallGuidanceTitle,
                break: 1,
              }),
            ],
            heading: HeadingLevel.HEADING_1,
          ...
          ...writeToRe(responseArray, indicesToPrint).map((responseToPrint) => {
            const paragraphs = [];

            paragraphs.push(new Paragraph({
              children: [
                new TextRun({
                  text: responseToPrint,
                  break: 1,
                }),
              ],
              style: "times",
...
function getResponseArray(language) {
  if (language==="English") {
    return [
      ...
      "Identity access management: users must only have access to the applications, functionality and data in line with their business needs. Too broad access rights can lead to data leaks and misuse.",

      "Management of Admin accounts: administrator accounts can provide unlimited access to systems and applications and only a few employees, typically from the IT department, should have access to them. Proper management of these admin accounts and their passwords is important, especially when third parties, e.g. IT suppliers, also have access to these accounts.",
      ...
      "Security logging: Using security logging at network and application level, actions are recorded so that suspicious or unauthorised actions can be identified and appropriate action can be taken in a timely manner.",

      "Incident response: Be prepared to respond to cyber incidents from the moment of the first detection of an attack.",

      "Disaster recovery plan: Be prepared to rebuild your entire IT environment from zero in the event of serious calamities, which also include a major cyber-attack.",
      ...
      "Encryption: The use of encryption to secure data, e.g. data base encryption, email traffic, data carriers.",

    ];
  } else {
    return ["Gerenciamento de senhas: senhas fortes são a base da segurança e devem ser obrigatórias para todos os usuários da organização. As senhas devem ser suficientemente longas e complexas, escolhidas pelos próprios usuários e alteradas regularmente. Após várias tentativas usando senhas incorretas, a conta deve ser bloqueada temporariamente. A importância de senhas fortes, que não devem ser compartilhadas com outras pessoas, deve ser enfatizada para os usuários.",

      "Gerenciamento de acesso de identidade: os usuários devem ter acesso apenas aos aplicativos, funcionalidades e dados de acordo com suas necessidades na organização. Direitos de acesso muito amplos podem levar a vazamentos de dados e uso indevido.",

      "Gerenciamento de contas de administrador: as contas de administrador podem fornecer acesso ilimitado a sistemas e aplicativos e apenas alguns funcionários, normalmente do departamento de TI, devem ter acesso a eles. O gerenciamento adequado dessas contas de administrador e suas senhas é importante, especialmente quando terceiros, por exemplo , fornecedores de TI, também têm acesso a essas contas.",

      "Gerenciamento da segurança do sistema operacional: por exemplo , a segurança do ambiente Windows é gerenciada pelo Active Directory. Como o Windows é um sistema operacional comum em muitas organizações e o Active Directory especifica o acesso, o gerenciamento adequado disso é importante.",

      "Backup e recuperação: Boas ferramentas e procedimentos devem ser implementados para que os dados e sistemas críticos sejam copiados em intervalos regulares e que possam ser restaurados em caso de problema.",
      ...
      "Treinamento de conscientização: os colaboradores são o elo mais fraco na segurança cibernética, pois cometem erros, não podem ser 'corrigidas' e seu comportamento e hábitos são difíceis de mudar. Os mesmos devem, portanto, ser treinados para entender a importância da segurança cibernética e reconhecer situações perigosas para que possam ser orientados para um comportamento seguro.",
      ...
      "Segurança de rede e acesso remoto para colaboradores e parceiros: sua rede deve ser gerenciada para que apenas dispositivos autorizados possam acessar a rede e dispositivos não autorizados sejam impedidos de acessar a rede.",

      "Registro de segurança: Usando o registro de segurança no nível da rede e e de aplicativos, as ações são registradas para que ações suspeitas ou não autorizadas possam ser identificadas e medidas apropriadas possam ser tomadas em tempo hábil.",

      "Resposta a incidentes: Esteja preparado para responder a incidentes cibernéticos desde o momento da primeira detecção de um ataque.",

      "Plano de recuperação de desastres: Esteja preparado para reconstruir todo o seu ambiente de TI do zero em caso de calamidades graves, que também incluem um grande ataque cibernético.",

      "Controle de inventário de hardware e software: Gerenciamento ativo, ou seja , inventário, acompanhamento e correção de todos os ativos de software e hardware dentro da rede da organização para que apenas software e hardware autorizados sejam permitidos.",

      "Autenticação multifatorial: uso de várias técnicas de autenticação além de senhas (impressão digital, reconhecimento facial, cartão de identificação, token etc.) para verificar a identidade do usuário.",

      "Testes de penetração periódicos: Com isso, um ataque cibernético externo controlado é realizado a pedido do cliente dentro de uma estrutura formalmente acordada., usando técnicas e ferramentas que os hackers usam. Isso permite avaliar quais danos podem ser causados por um hacker e como isso pode ser evitado.",

      "Monitoramento de segurança; O monitoramento contínuo da rede e dos sistemas da organização facilita a identificação oportuna de atividades suspeitas com base em sinais e padrões para permitir que a organização responda adequadamente. Isso inclui monitoramento de end-points, servidor e rede.",

      "Segurança de banco de dados e proteção de software operacional: o uso de ferramentas e processos avançados para impedir o acesso direto aos dados, a fim de garantir a confidencialidade e a integridade das informações confidenciais.",

      "Proteção de aplicativos: Garantindo a segurança ideal para aplicativos sensíveis, por meio do qual as configurações e senhas padrão são modificadas.",

      "Criptografia: O uso de criptografia para proteger dados, por exemplo , criptografia de banco de dados, tráfego de e-mail, suportes de dados.",
...
exports.sendEmailOnAssessmentsCompletion = functions.https.onCall(async (data, context) => {
  try {
    if (!context.auth) {
      throw new functions.https.HttpsError("internal", "The user is not authenticated. Only authenticated Admin users can create new users.");
    }

    const callerUid = context.auth.uid;

    const existingUserDoc = await admin.firestore().collection("users").doc(callerUid).get();
    if (!existingUserDoc.exists) {
      throw new functions.https.HttpsError("internal", "User exists in the auth but not in database");
    }

    const existingUser = existingUserDoc.data();
    if (!existingUser.isPrivacySurveyCompleted && !existingUser.isSecuritySurveyCompleted) {
      return `The user ${existingUser.fullName} - ${existingUser.company} hasn't completed both assessments yet`;
    }
    let emailSent = false;
    if (!existingUser.dashboardEmailSent) {
      let subject = "Access your Dashboard";
      let text = "You can now access the Dashboard via this link: https://app.cyberseguroglobal.com/Dashboard.";
      await _sendEmail(subject, text, existingUser.email);
      emailSent = true;
    }

    const updateUser = {
      dashboardEmailSent: emailSent,
      displayDashboardNotification: true,
      updatedAt: FieldValue.serverTimestamp(),
    };

    await admin.firestore().collection("users").doc(callerUid).update(updateUser);
...
exports.updateDashboardNotification = functions.https.onCall(async (data, context) => {
  try {
    if (!context.auth) {
      throw new functions.https.HttpsError("internal", "The user is not authenticated. Only authenticated Admin users can create new users.");
    }
    ...
    const callerUid = context.auth.uid;
    const value = data.displayDashboardNotification;
    ...
    const updateUser = {
      displayDashboardNotification: value,
      updatedAt: FieldValue.serverTimestamp(),
    };
...
async function _sendEmail(subject, text, receiverID) {
  const gmailEmail = functions.config().gmail.email;
  const gmailPassword = functions.config().gmail.password;
  const nodemailer = require("nodemailer");
...
exports.sendCyberInsuranceEmail = functions.https.onCall(async (data, context) => {
  try {
    const nodemailer = require("nodemailer");
    const fs = require("fs");
    const os = require("os");
    ...
    const emailTo = "ilesh.dattani@assentian.com";
    const emailCC = "abdul.fatir@assentian.com";
    ...
    const existingUser = existingUserDoc.data();
    const usersRef = admin.database().ref("/combinedusers/" + callerUid);
    const snapshot = await usersRef.once("value");
    const dbUser = snapshot.val();
    const answers = dbUser.cyberInsuranceQuestionnaire.answers;

    const sections = {
      english: {
        "sectionTitles": [
          "Client Data ('YOU')",
          "Questions for Clients with Annual Revenue from zero to BRL 100 million (ALL)",
          "Question for Clients with Annual Revenue between BRL 50 million and BRL 100 million",
        ],
        "Client Data ('YOU')": {
          ...
          revenueFromUSA: "% of revenue coming from USA",
        },
        "Questions for Clients with Annual Revenue from zero to BRL 100 million (ALL)": [
          {
            question: "Q1. Do you have commercial-grade firewalls on all external connections (gateways) on your network and commercial-grade antivirus throughout your network, including servers and endpoints?",
            answer: `Ans. ${yesNoAnswer(answers[1], "en")}`,
          },
          {
            question: "Q2. Do you or your IT Service Provider back up the data necessary to carry out your professional activities, at least every 7 days?",
            answer: `Ans. ${yesNoAnswer(answers[2], "en")}`,
          },
          {
            question: "Q3. Is backed-up data stored in an environment that is separate from your network and is integrity tested at least every 180 days?",
            answer: `Ans. ${yesNoAnswer(answers[3], "en")}`,
          },
          {
            question: "Q4. Do you install critical security updates (critical patches) within 30 days of the date they are published?",
            answer: `Ans. ${yesNoAnswer(answers[4], "en")}`,
          },
          {
            question: "Q5. Do you password or biometrically protect all portable devices such as laptops, tablets, smartphones, flash drives, portable hard drives/SSDs and memory cards/chips?",
            answer: `Ans. ${yesNoAnswer(answers[5], "en")}`,
          },
          {
            question: "Q6. Are you compliant with Payment Industry Security Standards (PCI)(*)?",
            answer: `Ans. ${yesNoAnswer(answers[6], "en")}`,
            additionalText: "(*) Questions about PCI? Access: https://pt.pcisecuritystandards.org/",
          },
          {
            question: "Q7. Have you (i) suffered a loss or (ii) a claim has been brought against you or (iii) are you aware of any fact or circumstance that could reasonably give rise to a loss or claim that is covered by comprehensive cyber insurance?",
            answer: `Ans. ${yesNoAnswer(answers[7], "en")}`,
            additionalText: "Simplified Questionnaire | Comprehensive Cyber Insurance Companies with Annual Revenue (Revenue) up to BRL 200,000,000.00",
          },
        ],
        "Question for Clients with Annual Revenue between BRL 50 million and BRL 100 million": [
          {
            question: "Q8. Do you have secure remote access to the applications you need to carry out your professional activities protected with at least 2-factor authentication?",
            answer: `Ans. ${yesNoAnswer(answers[8], "en")}`,
          },
          {
            question: "Q9. Is the Remote Desktop Protocol (RDP) disabled on all access points (endpoints) where it is not required, including servers?",
            answer: `Ans. ${yesNoAnswer(answers[9], "en")}`,
          },
          {
            question: "Q10. Is all Personal and/or Sensitive Data(*) always encrypted while stored on, or transmitted from, your network?",
            answer: `Ans. ${yesNoAnswer(answers[10], "en")}`,
          },
          {
            question: "Q11. Do you protect remote access to your network and the Personal and/or Sensitive Data(*) stored on it with at least 2-factor authentication?",
            answer: `Ans. ${yesNoAnswer(answers[11], "en")}`,
            additionalText: "(*) As defined by LF 13.709/18 and normative acts that may change or replace it",
          },
        ],
      },
      portuguese: {
        "sectionTitles": [
          "Dados do Proponente ('Você')",
          "Perguntas para Proponentes com ReceitaAnual de zero até R$ 100 milhões (TODOS)",
          "Pergunta para Proponentes com ReceitaAnual entre R$ 50 milhões e R$ 100 milhões",
          "Perguntas para Proponentes com ReceitaAnual de R$ 100 milhões a R$ 200 milhões",
        ],
        "Dados do Proponente ('Você')": {
          ...
          revenueFromUSA: "% da receita que vem dos EUA",
        },
        "Perguntas para Proponentes com ReceitaAnual de zero até R$ 100 milhões (TODOS)": [
          {
            question: "Q1. Vocêpossuifirewalls de nívelcomercialemtodas as conexõesexternas (gateways) de sua rede e antivírusde nívelcomercialemtodasua rede, inclusive servidores e terminais (endpoints)?",
            answer: `Ans. ${yesNoAnswer(answers[1], "pt-BR")}`,
          },
          {
            question: "Q2. VocêouseuPrestador de Serviços de TI efetuabackup dos dados necessários para realizarsuasatividadesprofissionais, aomenos a cada 7 dias?",
            answer: `Ans. ${yesNoAnswer(answers[2], "pt-BR")}`,
          },
          {
            question: "Q3. Os dados gravadosembackup sãoarmazenadosem um ambiente que é separado da sua rede e temsuaintegridadetestadaaomenos a cada 180 dias?",
            answer: `Ans. ${yesNoAnswer(answers[3], "pt-BR")}`,
          },
          {
            question: "Q4. Vocêinstalaatualizações de segurançacríticas (patches críticos) ematé 30 dias da data em que sãopublicados?",
            answer: `Ans. ${yesNoAnswer(answers[4], "pt-BR")}`,
          },
          {
            question: "Q5. Você protege com senhaoubiometriatodososdispositivosportáteis, taiscomolaptops, tablets, smartphones, pen drives, HDs/SSDs portáteis e cartões/chips de memória?",
            answer: `Ans. ${yesNoAnswer(answers[5], "pt-BR")}`,
          },
          {
            question: "Q6. Vocêestáemconformidade com ospadrões de segurança da indústria de pagamento (PCI)(*)?",
            answer: `Ans. ${yesNoAnswer(answers[6], "pt-BR")}`,
            additionalText: "(*) Dúvidassobre PCI? Acesse: https://pt.pcisecuritystandards.org/",
          },
          {
            question: "Q7. Você (i) sofreualgumaperdaou (ii) foipropostaalgumareclamação contra vocêou (iii) vocêpossuiconhecimento de algumfatooucircunstância que razoavelmentepossadarorigem a umaperdaoureclamaçãopassível de coberturaem um segurocompreensivocibernético?",
            answer: `Ans. ${yesNoAnswer(answers[7], "pt-BR")}`,
            additionalText: "QuestionárioSimplificado | Seguro CompreensivoCibernético Empresas com Receita (Faturamento) Anualaté R$ 200.000.000,00",
          },
        ],
        "Pergunta para Proponentes com ReceitaAnual entre R$ 50 milhões e R$ 100 milhões": [
          {
            question: "Q8. Vocêpossuiacessoremotoseguroàsaplicações que sãonecessárias para realizarsuasatividadesprofissionaisprotegido com, no mínimo, autenticação de 2 fatores?",
            answer: `Ans. ${yesNoAnswer(answers[8], "pt-BR")}`,
          },
        ],
        "Perguntas para Proponentes com ReceitaAnual de R$ 100 milhões a R$ 200 milhões": [
          {
            question: "Q9. O protocolo de acessoremoto (Remote Desktop Protocol – RDP) estádesabilitadoemtodosospontos de acesso (endpoints) ondeelenão é requerido, incluindoservidores?",
            answer: `Ans. ${yesNoAnswer(answers[9], "pt-BR")}`,
          },
          {
            question: "Q10. Todos Dados Pessoais e/ouSensíveis(*) estão sempre criptografadosenquantoestãoarmazenadosem, outransmitidos a partir da sua rede?",
            answer: `Ans. ${yesNoAnswer(answers[10], "pt-BR")}`,
          },
          {
            question: "Q11. Você protege o acessoremoto à sua rede e aos Dados Pessoais e/ouSensíveis(*) nelaarmazenados com, no mínimo, autenticação de 2 fatores?",
            answer: `Ans. ${yesNoAnswer(answers[11], "pt-BR")}`,
          },
        ],
      },
    };

    const companyName = existingUser.company;
    const compName = companyName.toLowerCase()
        .replace(/(^|\s)\S/g,
            function(x) {
              return x.toUpperCase();
    ...
    const path = require("path");
    const fileName = companyName.toUpperCase() +" Cyber Insurance.docx";
    const fileNamePortuguese = companyName.toUpperCase() +" Cyber Insurance Portuguese.docx";
    const tempFilePath = path.join(os.tmpdir(), fileName);
    const tempFilePathPortuguese = path.join(os.tmpdir(), fileNamePortuguese);
    ...
    const mailOptions = {
      ...
      to: emailTo,
      cc: emailCC,
      subject: `Cyber Insurance Application for ${capitalizeFirstLetter(companyName)}`,
      text: `Dear Ilesh, \nAttached is the Cyber Insurance Application for ${companyName}.\nBest Regards, \nCyber Seguro Global`,
      attachments: [{
        path: tempFilePath,
      }, {
        path: tempFilePathPortuguese,
    ...
    return "Sending Cyber Insurance Email...";
  } catch (error) {
...
const yesNoAnswer = (answer, language) => {
  if (!answer) {
    return language === "pt-BR" ? "Não" : "No";
  }
  if (language === "pt-BR") {
    return answer === 1 ? "Sim" : "Não";
  }
    ...
    sections: [
      {
        ...
        children: [
          ...
          createSectionHeading(sections["sectionTitles"][0]),
          createQuestion(sections[sections["sectionTitles"][0]]["companyName"]),
          createAnswer(client["companyName"]),

          createQuestion(sections[sections["sectionTitles"][0]]["sector"]),
          createAnswer(client["sector"]),

          createQuestion(sections[sections["sectionTitles"][0]]["address"]),
          createAnswer(client["address"]),

          createQuestion(sections[sections["sectionTitles"][0]]["registrationNumber"]),
          createAnswer(client["registrationNumber"]),

          createQuestion(sections[sections["sectionTitles"][0]]["website"]),
          createAnswer(client["website"]),

          createQuestion(sections[sections["sectionTitles"][0]]["annualRevenue"]),
          createAnswer(client["annualRevenue"]),

          createQuestion(sections[sections["sectionTitles"][0]]["revenueFromUSA"]),
          createAnswer(client["revenueFromUSA"]),

          ...buildSections(sections).reduce((prev, curr) => prev.concat(curr), []),
