...
const AdminPrivacyAndDataProtectionDashboardPage = ({location, history}) => {
    ...
    const getAvgCostOverTimeByCompanySize = async (companySize) => {
        ...
        if (avgCostOverTimeByCompanySizeRef.exists) {
            ...
            data.data.forEach(obj => {
                tempOptions.xAxis.categories.push(obj.title);
                tempOptions.series[0].data.push(obj.value)
            });
...
export default withAuthorization(authCondition)(withRouter(AdminPrivacyAndDataProtectionDashboardPage));
